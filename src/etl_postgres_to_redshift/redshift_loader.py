...
def create_table_if_not_exists(table_name: str, columns: dict, config: dict):
    schema = config["redshift"].get("schema", "public")
    ...
    try:
        with get_redshift_connection(connection_string) as conn:
            with conn.cursor() as cur:
                cur.execute(query)
                conn.commit()
                logger.info(f"Ensured table {schema}.{table_name} exists.")
        ...
        with get_redshift_connection(connection_string) as conn:
            with conn.cursor() as cur:
                for i in range(0, len(values), batch_size):
                    ...
                    conn.commit()
                    logger.info(f"Inserted batch {i} to {i+len(batch)} into {schema}.{table_name}")
        logger.info(f"✅ Data loaded into Redshift table {schema}.{table_name}")
    except Exception as e:
        logger.error(f"❌ Error loading data into {schema}.{table_name}: {e}", exc_info=True)
        raise
